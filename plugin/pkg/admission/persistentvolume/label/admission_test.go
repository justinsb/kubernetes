...
func TestAdmission(t *testing.T) {
	...
	err := handler.Admit(admission.NewAttributesRecord(&ignoredPV, "PersistentVolume", ignoredPV.Namespace, ignoredPV.Name, "persistentvolumes", "", admission.Create, nil))
	if err != nil {
		t.Errorf("Unexpected error returned from admission handler (on ignored pv): %v", err)
	}

	// We only add labels on creation
	err = handler.Admit(admission.NewAttributesRecord(&awsPV, "PersistentVolume", awsPV.Namespace, awsPV.Name, "persistentvolumes", "", admission.Delete, nil))
	if err != nil {
	...
	err = handler.Admit(admission.NewAttributesRecord(&awsPV, "PersistentVolume", awsPV.Namespace, awsPV.Name, "persistentvolumes", "", admission.Create, nil))
	if err == nil {
