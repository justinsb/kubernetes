...
function kube-up {
  ...
  # Masters and minions can talk freely
  authorize-security-group-ingress "${MASTER_SG_ID}" "--source-group ${MINION_SG_ID} --protocol -1"
  authorize-security-group-ingress "${MINION_SG_ID}" "--source-group ${MASTER_SG_ID} --protocol -1"
  ...
  if [[ -n "${vpc_id}" ]]; then
    ...
    for sg_id in ${sg_ids}; do
      ...
      for other_sgid in ${other_sgids}; do
        $AWS_CMD revoke-security-group-ingress --group-id "${sg_id}" --source-group "${other_sgid}" --protocol -1 > $LOG
      done
    done
...
function test-setup {
  ...
  authorize-security-group-ingress "${MINION_SG_ID}" "--protocol tcp,udp --port 30000-32767 --cidr 0.0.0.0/0"
