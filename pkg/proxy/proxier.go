...
func (proxier *Proxier) openPublicPort(publicPort int, protocol api.Protocol, proxyIP net.IP, proxyPort int, name ServicePortName) error {
	...
	existed, err := proxier.iptables.EnsureRule(iptables.TableNAT, iptablesExportChain, args...)
	if err != nil {
		glog.Errorf("Failed to install iptables %s rule for service %q", iptablesExportChain, name)
		return err
	}
	if !existed {
...
func (proxier *Proxier) closePublicPort(publicPort int, protocol api.Protocol, proxyIP net.IP, proxyPort int, name ServicePortName) []error {
	...
	args := proxier.iptablesPublicPortArgs(publicPort, protocol, proxyIP, proxyPort, name)
	if err := proxier.iptables.DeleteRule(iptables.TableNAT, iptablesExportChain, args...); err != nil {
		glog.Errorf("Failed to delete iptables %s rule for service %q", iptablesExportChain, name)
		el = append(el, err)
	}
...
// Chain for exported services
var iptablesExportChain iptables.Chain = "KUBE-EXPORT"
...
func iptablesFlush(ipt iptables.Interface) error {
	...
	if err := ipt.FlushChain(iptables.TableNAT, iptablesExportChain); err != nil {
		el = append(el, err)
	}
	if len(el) != 0 {
...
func (proxier *Proxier) iptablesPublicPortArgs(publicPort int, protocol api.Protocol, proxyIP net.IP, proxyPort int, service ServicePortName) []string {
	args := iptablesCommonPortalArgs(nil, 0, protocol, service)
