...
// GetNodeAddresses returns the NodeAddresses of a particular machine instance.
func (v *VagrantCloud) GetNodeAddresses(instance string) ([]api.NodeAddress, error) {
	// Due to vagrant not running with a dedicated DNS setup, we return the IP address of a minion as its hostname at this time
	minion, err := v.getInstanceByAddress(instance)
	if err != nil {
		return nil, err
	}
	return api.ConvertLegacyIPToNodeAddresses(minion.IP), nil
}
